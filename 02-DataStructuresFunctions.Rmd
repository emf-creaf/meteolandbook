...
+ Functions `readmeteorologygrid()` and `readmeteorologypixels()` read the meteorological data stored in a **NetCDF** file and return an object of class `SpatialGridMeteorology` or `SpatialPixelsMeteorology`, respectively. Users can read subsets of the time period stored in **NetCDF** by specifying parameter `dates`. Rotated grids should not be read using this functions.
...
1. It is possible to coerce an object of `SpatialPointsTopography` into a `SpatialPixelsTopography` using a call like `as(x, "SpatialPixelsTopography")`. 
2. Subsetting (i.e.`[` operator) is also allowed, so that estimations of daily meteorology can be performed in subsets of points or grids.
3. The package provides functions to extract meteorological data:
