...
3. The package provides functions to extract meteorological data (these allow reading data in memory or from the disk):
  ...
  + Function `extractdates()` extracts the meteorology of a set of dates from a `SpatialPointsMeteorology`, (or `SpatialGridMeteorology` or `SpatialPixelsMeteorology`) and returns a `SpatialPointDataFrame` (or `SpatialGridDataFrame` or `SpatialPixelsDataFrame`) for each date in a named list. This kind of conversion is also useful to export meteoland outputs to package [**raster**](https://cran.r-project.org/package=raster), which provides wrappers for the abovementioned **sp** classes.
